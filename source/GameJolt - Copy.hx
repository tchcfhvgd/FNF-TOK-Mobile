...
class GameJoltLogin extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        var logo:FlxSprite = new FlxSprite(-1070, -1100).loadGraphic(Paths.image('TheOrigamiKingLogo'));
        ...
        if(GameJoltAPI.getStatus())
        {
            username1 = new FlxText(0, 300, 0, "Signed in as:", 40);
            username1.alignment = CENTER;
            username1.screenCenter(X);
            username1.x += baseX;
            add(username1);

            username2 = new FlxText(0, 350, 0, "" + GameJoltAPI.getUserInfo(true) + "", 40);
